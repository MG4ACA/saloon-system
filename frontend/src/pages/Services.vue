<template>
  <div class="services-page">
      ...
      <div v-for="svc in services" :key="svc.id" class="svc-card">
        <div class="svc-card-top">
          ...
          <Tag
            ...
            :severity="svc.is_active ? 'success' : 'danger'"
          />
        </div>
        <div class="svc-card-meta">
          <span>LKR {{ Number(svc.basePrice).toFixed(2) }}</span>
          <span>{{ svc.duration }} min</span>
          <span v-if="svc.commissionType">
            {{
              svc.commissionType === 'percentage'
                ? svc.commissionValue + '%'
                : 'LKR ' + svc.commissionValue
            }}
          </span>
        </div>
    ...
    <Dialog
      ...
      :header="editTarget ? 'Edit Service' : 'Add Service'"
      :style="{ width: '540px' }"
      modal
    >
      <form @submit.prevent="submitForm">
        <div class="grid">
          ...
          <div class="col-6 field">
            ...
            <Select
              v-model="form.commission.commissionType"
...
const submitForm = async () => {
  ...
  try {
    ...
    formError.value = details
      ? details.join(' ')
      : e.response?.data?.error || 'Failed to save service.';
  } finally {
...
const toggleStatus = async (svc) => {
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Updated',
      detail: `Service ${svc.is_active ? 'deactivated' : 'activated'}.`,
      life: 3000,
    });
    await fetchData();
  } catch {
