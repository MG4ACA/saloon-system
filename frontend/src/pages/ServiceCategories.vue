  ...
  <div class="categories-page">
    ...
    <DataTable :value="categories" :loading="loading" stripedRows dataKey="id" class="p-datatable-sm desktop-table">
      ...
      <Column field="is_deleted" header="Status" style="width:8rem">
        <template #body="{ data }">
          <Tag :value="data.is_deleted ? 'Inactive' : 'Active'" :severity="data.is_deleted ? 'danger' : 'success'" />
        </template>
      </Column>
    ...
    <Dialog v-model:visible="showModal" :header="editTarget ? 'Edit Category' : 'Add Category'" :style="{ width: '420px' }" modal>
      <form @submit.prevent="submitForm">
        ...
        <div class="field">
          ...
          <Textarea v-model="form.description" placeholder="Optional description" class="w-full" rows="3" />
        </div>
...
const fetchCategories = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to load categories', life: 4000 });
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  ...
  try {
    ...
    formError.value = details ? details.join(' ') : (e.response?.data?.error || 'Failed to save category.');
  } finally {
...
const toggleStatus = async (cat) => {
  try {
    ...
    toast.add({ severity: 'success', summary: 'Updated', detail: `Category ${cat.is_deleted ? 'activated' : 'deactivated'}.`, life: 3000 });
    await fetchCategories();
  } catch (e) {
